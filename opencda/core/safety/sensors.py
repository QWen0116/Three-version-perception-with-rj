...
class CollisionSensor(object):
    ...
    def _on_collision(weak_self, event) -> None:
        ...
        if not self:
            return
        self.new_collision_detected = True
        impulse = event.normal_impulse
        self.intensity = math.sqrt(impulse.x ** 2 + impulse.y ** 2 + impulse.z ** 2)
        # print(f"Collision intensity: {self.intensity}, Threshold: {self._threshold}")
        self._history.append((event.frame, self.intensity))
        if self.intensity > self._threshold:
            self.collided = True
            self.collided_frame = event.frame
            self.last_collision_time = time.time()  # 更新最后碰撞时间
            print(f"Collision detected! Frame: {event.frame}, Intensity: {self.intensity}")
        else:
            self.collided = False
            print(f"No Collision! Frame: {event.frame}, Intensity: {self.intensity}")
    ...
    def tick(self, data_dict):
        # pass
        if self.collided:
            print(f"{self.intensity}")

        # Reset collision state unless a new collision has been detected
        time_since_collision = time.time() - self.last_collision_time
        if not self.new_collision_detected and time_since_collision > 30:
            self.collided = False
