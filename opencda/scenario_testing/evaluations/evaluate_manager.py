...
class EvaluationManager(object):
    ...
    def evaluate(self):
        ...
        self.safetystatus_eval(log_file)
        print('Safety_status Evaluation Done.')
    ...
    def localization_eval(self, log_file):
        ...
        for vid, vm in self.cav_world.get_vehicle_managers().items():
            ...
            # figure, perform_txt = loc_debug_helper.evaluate()
            #添加路程
            figure, perform_txt, gt_x, gt_y = loc_debug_helper.evaluate()
            ...
            gt_data.to_excel(excel_save_path, index=False)
    ...
    def safetystatus_eval(self, log_file):
        """
        Running module evaluation.

        Args:
            -log_file (File): The log file to write the data.
        """
        lprint(log_file, "***********Safety status Module***********")
        for vid, vm in self.cav_world.get_vehicle_managers().items():
            actor_id = vm.vehicle.id
            lprint(log_file, 'Actor ID: %d' % actor_id)
            ...
            excel_save_path = os.path.join(
                    self.eval_save_path,
                    '%d_safety_status.xlsx' %
                    actor_id)
            SafetyManager.save_status_records_to_excel(excel_save_path)



            counts = {'collision': 0, 'stuck': 0, 'offroad': 0, 'ran_light': 0}
            # 计算每个状态为True的次数
            for record in SafetyManager.status_records:
                for key in counts.keys():
                    if record.get(key, False):
                        counts[key] += 1


            total_records = len(SafetyManager.status_records)
            for key, count in counts.items():
                percentage = (count / total_records) * 100 if total_records > 0 else 0
                lprint(log_file, f"{key} True percentage: {percentage:.2f}%")
